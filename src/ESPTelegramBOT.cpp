...
/*
  Adapted from TelegramBot (Giancarlo Bacchio) - Library to create your 
  own Telegram Bot using ESP8266.
...
#define HOST "api.telegram.org"//sanusb.org
#define SSL_PORT 443
...
String TelegramBOT::connectToTelegram(String command)  {
    ...
    if (client.connect(HOST, SSL_PORT)) {  //SSL port
...
void TelegramBOT::getUpdates(String offset)  {
    ...
    if (mess!="") {
            ...
            for (int n=1; n<mess.length()+1; n++) {   //Search for each message start
                ...
                //Serial.print(c);//Print {"ok":true,"result":[]}
                a=a+c;
                if (ch_count>8) {
	...
	//Serial.println();        	
	for (int b=1; b<i+1; b++)  {
          Serial.println(message[b][0]); //imprime o json
        }
        Serial.println();
        analizeMessages();
    }
...
void TelegramBOT::analizeMessages(void)     {
  ...
  int rif[6]= {0,0,0,0,0,0}; //pointers in message json (update_id, first_name, lastname, chat_id, date, text)
  for (int i=1; i<message[0][0].toInt()+1; i++)      {
    ...
    for (int m=5; m<messageLenght+1; m++)             {
		...
		//if (a.substring(m-13,m)=="\"from\":{\"id\":")  { //Search for "from" pointer start
	      if (a.substring(m-7,m)=="\"date\":")         { //Search for "date" pointer start
          rif[4]=m;
        }
	    ...
	    rif[5]=m;
            ...
            if (a.substring(m-1,m)==",")  {              //Search for "update_id" pointers end
                if (rif[0]!=0)  {
                  message[i][0]=a.substring(rif[0],m-1);
                }
            rif[0]=0;
            }
        ...
        if (a.substring(m-1,m)==",")  {                 //Search for "first name" pointer end
              if (rif[1]!=0)  {
                message[i][1]=a.substring(rif[1],m-2);  //Write value into dedicated slot
              }
          rif[1]=0;
        }
        ...
        if (a.substring(m-1,m)==",")  {                 //Search for "chat_id" pointer end
              if (rif[3]!=0)  {
                message[i][3]=a.substring(rif[3],m-1);  //Write value into dedicated slot
              }
          rif[3]=0;
        }
		
		if (a.substring(m-1,m)==",")  {                //Search for   "date" pointers end
                if (rif[4]!=0)  {
        ...
        if (a.substring(m-2,m)=="\",")  {               //Search for "text" pointer end
              if (rif[5]!=0)  {
                message[i][5]=a.substring(rif[5],m-2);    //Write text value into dedicated slot  message[i][5]
            }
          rif[5]=0;
        }
    ...
    //for (int j=0; j<6; j++)	{
  	//Serial.println(message[i][j]);                    //imprime dados extraidos (analisados)
    //}
  }
}
